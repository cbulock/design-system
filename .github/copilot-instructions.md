...
- The split source files in `styles\fonts.css`, `styles\tokens.css`, and `styles\base.css` are the source of truth for the reusable system. `cindor.css` is the canonical packaged bundle, and `colors_and_type.css` is the compatibility wrapper used by the local preview pages and dashboard demo.
...
- Theme state is driven by `data-theme` on `document.documentElement`, with `cindor.theme` and `cindor.nav` persisted in `localStorage` by the dashboard demo.
...
- For reusable assets, treat `styles\fonts.css`, `styles\tokens.css`, and `styles\base.css` as the source files, and keep `cindor.css` and `colors_and_type.css` aligned as aggregate entrypoints.
